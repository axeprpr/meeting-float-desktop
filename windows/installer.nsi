...
Section "Install"
  ...
  CreateShortcut "$SMPROGRAMS\Meeting Float\Meeting Float.lnk" "$WINDIR\System32\wscript.exe" "$INSTDIR\${APP_EXE}"
  CreateShortcut "$SMPROGRAMS\Meeting Float\Mock Autotest.lnk" "$WINDIR\System32\wscript.exe" "$INSTDIR\autotest.vbs"
  CreateShortcut "$SMPROGRAMS\Meeting Float\Uninstall.lnk" "$INSTDIR\Uninstall.exe"
  CreateShortcut "$DESKTOP\Meeting Float.lnk" "$WINDIR\System32\wscript.exe" "$INSTDIR\${APP_EXE}"
